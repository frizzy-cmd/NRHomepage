...
document.addEventListener('DOMContentLoaded', () => {
	...
	if (msgForm) {
		msgForm.addEventListener('submit', async (e) => {
			...
			const authorVal = msgAuthor.value.trim() || 'Anonymous';
			...
			try {
				...
				if (res.ok) {
					...
					alreadyPostedNotice.style.display = 'block';
					loadMessages();
				} else {
					alert(data.error || 'Submission failed.');
				}
			} catch (err) {
				alert('ERROR ON SUBMIT MSG OOPS: ' + err.message);
			} finally {
				sendMsgBtn.disabled = false;
				sendMsgBtn.textContent = 'Send OneMessage';
			}
		});
	}
	...
	async function loadMessages() {
		try {
			...
			if (!msgs.length) {
				messagesStream.innerHTML = '<p style="color: #aaa;">No OneMessages written yet. Be the first!</p>';
				return;
			}
			...
			messagesStream.innerHTML = msgs.map(m => `
				<div class="msg-card">
					...
					<div class="msg-body">
						...
						<div>
							<button class="react-btn" onclick="reactMessage('${m.id}', 'light', this)">Give Light (${m.lights_count || 0})</button>
							<button class="react-btn" onclick="reactMessage('${m.id}', 'pancake', this)">Give Pancakes (${m.pancakes_count || 0})</button>
						</div>
					</div>
				</div>
			`).join('');
	...
	window.reactMessage = async function(msgId, reaction, btnEl) {
		try {
